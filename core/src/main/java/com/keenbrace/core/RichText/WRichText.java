package com.keenbrace.core.RichText;
...
public class WRichText {
    ...
    private static void setDrawable(Span span, SpannableString ss,int start, int end){
        Drawable drawable = span.getDrawable();
        if (drawable != null){
            Drawable d = drawable;
            d.setBounds(0, 0, d.getIntrinsicWidth(), d.getIntrinsicHeight());
            //用这个drawable对象代替字符串easy
            ImageSpan spand = new ImageSpan(d, ImageSpan.ALIGN_BASELINE);
            ss.setSpan(
                    spand,
                    start,
                    end,
